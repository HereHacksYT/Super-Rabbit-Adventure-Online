...
const groundTexture = createCanvasTexture(512, 512, (ctx, w, h) => {
    ...
    for (let i = 0; i < 8000; i++) {
        ...
        ctx.fillRect(Math.random()*w, Math.random()*h, 1 + Math.random() * 3, 2 + Math.random() * 5);
    }
});
...
function createEnclosingWalls(minX, minZ, maxX, maxZ, h = 25) {
    ...
    for (let i = 0; i < Math.ceil(wX/seg); i++) createMossyWallSegment(minX + (i+0.5)*(wX/Math.ceil(wX/seg)), maxZ, wX/Math.ceil(wX/seg)+1, h, 2, 0);
    for (let i = 0; i < Math.ceil(wX/seg); i++) createMossyWallSegment(minX + (i+0.5)*(wX/Math.ceil(wX/seg)), minZ, wX/Math.ceil(wX/seg)+1, h, 2, 0);
    for (let i = 0; i < Math.ceil(wZ/seg); i++) createMossyWallSegment(minX, minZ + (i+0.5)*(wZ/Math.ceil(wZ/seg)), wZ/Math.ceil(wZ/seg)+1, h, 2, Math.PI/2);
    for (let i = 0; i < Math.ceil(wZ/seg); i++) createMossyWallSegment(maxX, minZ + (i+0.5)*(wZ/Math.ceil(wZ/seg)), wZ/Math.ceil(wZ/seg)+1, h, 2, Math.PI/2);
}
...
for (let row = -100; row <= 100; row += 30)
    for (let col = -100; col <= 100; col += 30)
        if (Math.abs(row) > 30 || Math.abs(col) > 30) createBigTree(rfCenterX+col, rfCenterZ+row, 1.5+Math.random());
...
function createMonkey(x, y, z) {
    ...
    const fm = new THREE.MeshStandardMaterial({ color: 0x6B4226, roughness: 0.6 });
    const bm = new THREE.MeshStandardMaterial({ color: 0x8B5A3C, roughness: 0.6 });
    ...
    g.add(new THREE.Mesh(new THREE.CylinderGeometry(0.1,0.12,0.6,6), bm)).position.set(0.2,0.4,0.05);
    const body = new THREE.Mesh(new THREE.CylinderGeometry(0.35,0.4,1,8), bm);
    ...
    banana.add(new THREE.Mesh(new THREE.TubeGeometry(bc, 8, 0.04, 6, false), new THREE.MeshStandardMaterial({ color: 0xFFE135, roughness: 0.4 })));
    g.add(banana);
    const head = new THREE.Mesh(new THREE.SphereGeometry(0.3,8,8), new THREE.MeshStandardMaterial({ color: 0xA0704A, roughness: 0.5 }));
    head.position.y = 1.5; head.castShadow = true; head.receiveShadow = true; g.add(head);
    const ear = new THREE.SphereGeometry(0.1,6,6); const em = new THREE.MeshStandardMaterial({ color: 0xD4956B, roughness: 0.5 });
    g.add(new THREE.Mesh(ear, em)).position.set(-0.25,1.65,0);
    g.add(new THREE.Mesh(ear, em)).position.set(0.25,1.65,0);
    const eye = new THREE.SphereGeometry(0.06,6,6); const eyeMat = new THREE.MeshBasicMaterial({ color: 0x000000 });
    g.add(new THREE.Mesh(eye, eyeMat)).position.set(-0.1,1.6,0.25);
    g.add(new THREE.Mesh(eye, eyeMat)).position.set(0.1,1.6,0.25);
    ...
    const tail = new THREE.Mesh(new THREE.CylinderGeometry(0.04,0.06,0.5,6), bm);
    tail.position.set(0,0.5,-0.35); tail.rotation.x = 0.6; g.add(tail);
...
// Maymun sağlık barı
const mhc = document.createElement('div');
mhc.id = 'monkey-health'; mhc.style.cssText = 'display:none; position:absolute; top:55px; left:15px; z-index:5; width:180px; height:15px; background:rgba(0,0,0,0.6); border-radius:7px; border:1px solid white; overflow:hidden;';
...
function createRabbitStatue() {
    ...
    g.add(new THREE.Mesh(new THREE.BoxGeometry(0.7,0.75,0.75), wm)).position.y = 0.4;
    g.add(new THREE.Mesh(new THREE.BoxGeometry(0.55,0.55,0.55), wm)).position.set(0,0.95,0.1);
    g.add(new THREE.Mesh(new THREE.BoxGeometry(0.12,0.55,0.06), wm)).position.set(-0.16,0.45,-0.05);
    g.add(new THREE.Mesh(new THREE.BoxGeometry(0.12,0.55,0.06), wm)).position.set(0.16,0.45,-0.05);
...
function createCage(x, z) {
    const g = new THREE.Group();
    const bm = new THREE.MeshStandardMaterial({ color: 0x444444, roughness: 0.5, metalness: 0.3 });
    const floor = new THREE.Mesh(new THREE.BoxGeometry(3,0.2,3), bm);
    floor.position.y = 0.1; floor.receiveShadow = true; g.add(floor); obstacles.push(floor);
    for (let i = 0; i < 8; i++) {
        const a = (i/8)*Math.PI*2;
        const bar = new THREE.Mesh(new THREE.CylinderGeometry(0.08,0.08,3,8), bm);
        bar.position.set(Math.cos(a)*1.3, 1.7, Math.sin(a)*1.3); bar.castShadow = true; g.add(bar); obstacles.push(bar);
    }
    ...
    const lock = new THREE.Mesh(new THREE.BoxGeometry(0.3,0.3,0.3), new THREE.MeshStandardMaterial({ color: 0xff0000, roughness: 0.3, emissive: 0x330000 }));
    lock.position.set(0,1.8,1.35); lock.name = 'lock'; g.add(lock);
...
createBigGrassBlock(30, -25, 8, 8, 8); createBigGrassBlock(-30, -15, 9, 10, 6);
createBigGrassBlock(30, 25, 10, 8, 7); createBigGrassBlock(-30, -30, 9, 9, 10); createBigGrassBlock(25, 0, 8, 8, 6);
createBigTree(-35, -35, 2); createBigTree(35, -30, 1.8); createBigTree(-30, 35, 2.2); createBigTree(35, 35, 2);
createBigTree(-40, 10, 1.8); createBigTree(40, -10, 2); createBigTree(-40, -15, 2); createBigTree(40, 15, 2);
createBigTree(-15, -40, 1.8); createBigTree(15, 40, 1.8);
...
function createRabbitModel(isLocal) {
    ...
    const m = isLocal ? bodyMat : otherBodyMat;
    ...
    const eg = new THREE.BoxGeometry(0.07,0.07,0.07);
    h.add(new THREE.Mesh(eg, eyeMat)).position.set(-0.18,0.1,0.25);
    h.add(new THREE.Mesh(eg, eyeMat)).position.set(0.18,0.1,0.25);
    const ear = new THREE.BoxGeometry(0.12,0.55,0.06);
    h.add(new THREE.Mesh(ear, m)).position.set(-0.16,0.45,-0.05);
    h.add(new THREE.Mesh(ear, m)).position.set(0.16,0.45,-0.05);
...
function updateHealthBar() {
    const p = (myHealth/100)*100;
    document.getElementById('health-bar-fill').style.width = p + '%';
    document.getElementById('health-text').innerText = myHealth + '/100';
}
function die() {
    if (isDead) return;
...
function respawn() {
    isDead = false; gameActive = true; rabbit.visible = true; rabbit.position.set(0,0,0); rabbit.rotation.y = 0;
    myHealth = 100; updateHealthBar(); document.getElementById('death-screen').style.display = 'none';
    velocityY = 0; jumpCount = 0;
}
function checkCollision(nx, ny, nz) {
    if (!gameActive) return false;
    const box = new THREE.Box3(new THREE.Vector3(nx-0.28, ny+0.15, nz-0.28), new THREE.Vector3(nx+0.28, ny+1.1, nz+0.28));
    gameplayGroup.updateMatrixWorld(true);
    for (const o of obstacles) {
        if (!o || !o.parent) continue;
        const ob = new THREE.Box3().setFromObject(o);
        if (box.intersectsBox(ob) && ny < ob.max.y - 0.3) return true;
    }
    return false;
}
function getFloorY(px, py, pz) {
    let h = 0; gameplayGroup.updateMatrixWorld(true);
    for (const o of obstacles) {
        if (!o || !o.parent) continue;
        const b = new THREE.Box3().setFromObject(o);
        if (px+0.25 >= b.min.x && px-0.25 <= b.max.x && pz+0.25 >= b.min.z && pz-0.25 <= b.max.z && py >= b.max.y-0.4 && b.max.y > h) h = b.max.y;
    }
    return h;
}
...
function setupLobbyUI(d) {
    ...
    document.getElementById('ui-player-count').innerText = `Oyuncu: ${Object.keys(d.players).length} / ${maxPlayersLimit}`;
    ...
    rabbit.position.set(pp[0].x, 0.2, pp[0].z);
    for (const id of Object.keys(otherPlayers)) scene.remove(otherPlayers[id].mesh);
    otherPlayers = {};
    let pi = 1;
    for (const id of Object.keys(d.players)) if (id !== socket.id && pi < 4) { const pos = pp[pi]; addOtherPlayer(id, pos.x, 0.2, pos.z); pi++; }
}
socket.on('gameStartedAtAll', (ap) => {
    ...
    rabbit.position.set(0,0,0); myHealth = 100; updateHealthBar();
    for (const id of Object.keys(otherPlayers)) scene.remove(otherPlayers[id].mesh);
    otherPlayers = {};
    for (const id of Object.keys(ap)) if (id !== socket.id) addOtherPlayer(id, 0, 0, 0);
    gameActive = true; isDead = false;
});
function addOtherPlayer(id, x, y, z) {
    if (otherPlayers[id]) return;
    const m = createRabbitModel(false); m.mesh.position.set(x, y, z); scene.add(m.mesh);
    otherPlayers[id] = { mesh: m.mesh, visual: m.visual, head: m.head, isAttacking: false, attackAnimTime: 0 };
}
...
window.addEventListener('touchstart', (e) => {
    if (e.touches.length === 1 && !zone.contains(e.target) && !document.getElementById('jump-button').contains(e.target) && !document.getElementById('attack-button').contains(e.target)) {
...
function updateMonkeys(dt) {
    ...
    for (const mk of monkeys) {
        ...
        if (d < ud.chaseRange && dh < ud.homeRange) {
            ...
            ud.targetX = mk.position.x + Math.sin(a) * ud.speed;
            ud.targetZ = mk.position.z + Math.cos(a) * ud.speed;
            mk.rotation.y = a;
        ...
        mk.position.x += (ud.targetX - mk.position.x) * 0.1;
        mk.position.z += (ud.targetZ - mk.position.z) * 0.1;
        mk.position.y = ud.homeY + Math.abs(Math.sin(Date.now()*0.005 + ud.homeX)) * 0.2;
        if (d < ud.chaseRange) {
            mhc.style.display = 'block';
            document.getElementById('mhf').style.width = (ud.health/ud.maxHealth*100) + '%';
            document.getElementById('mht').innerText = ud.health + '/' + ud.maxHealth;
        }
        if (isAttacking && d < ud.attackRange + 0.5 && attackAnimTime < 0.2) {
            ud.health -= 25;
            if (ud.health <= 0) { ud.health = 0; ud.isDead = true; ud.deathTime = Date.now(); mk.visible = false; mhc.style.display = 'none'; }
        }
        allDead = allDead && ud.isDead;
    }
    if (allDead && !hasKey && keyMesh) keyMesh.visible = true;
    if (keyMesh && keyMesh.visible && !hasKey && rabbit.position.distanceTo(keyMesh.position) < 2) {
        hasKey = true; keyMesh.visible = false; keyMesh.position.set(0,0,0); keyMesh.scale.set(0.5,0.5,0.5);
        rabbit.add(keyMesh); keyMesh.position.set(0, 2, 0); keyMesh.visible = true;
    }
    if (hasKey && cageGroup && !cageOpened && rabbit.position.distanceTo(cageGroup.position) < 3) {
        cageOpened = true;
        for (const c of [...cageGroup.children]) { const i = obstacles.indexOf(c); if (i > -1) obstacles.splice(i, 1); }
        gameplayGroup.remove(cageGroup); cageGroup = null;
        if (keyMesh) { rabbit.remove(keyMesh); keyMesh = null; hasKey = false; }
        const msg = document.createElement('div');
        msg.style.cssText = 'position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); color:white; font-size:48px; font-weight:bold; text-shadow:0 0 20px gold; z-index:100;';
        msg.innerText = 'Pamuk Kurtuldu!'; document.body.appendChild(msg);
        setTimeout(() => { msg.style.transition = 'opacity 1s'; msg.style.opacity = '0'; setTimeout(() => document.body.removeChild(msg), 1000); }, 2000);
    }
}
...
function animate() {
    ...
    updateMonkeys(dt);
    let mx = 0, mz = 0;
    if (joystickActive) { mx = moveX; mz = moveZ; }
    ...
    if (gameActive && !isDead) {
        for (const p of portals) if (new THREE.Vector2(rabbit.position.x - p.mesh.position.x, rabbit.position.z - p.mesh.position.z).length() < 2.5 && Date.now()/1000 - lastTeleportTime > teleportCooldown) {
            lastTeleportTime = Date.now()/1000;
            document.getElementById('death-screen').style.display = 'flex';
            ...
            setTimeout(() => { rabbit.position.x = p.target.x; rabbit.position.z = p.target.z; document.getElementById('death-screen').style.display = 'none'; }, 600);
            break;
        }
    }
    if (isOnlineMode && !gameActive && !isDead) {
        rabbit.rotation.y += 1.2 * dt;
        for (const id of Object.keys(otherPlayers)) otherPlayers[id].mesh.rotation.y += 1.2 * dt;
    ...
    if (gameActive && !isDead) {
        ...
        if (isAttacking) {
            attackAnimTime += 12*dt; const f = Math.sin(attackAnimTime*Math.PI);
            if (attackAnimTime <= 1) { player.visual.position.z = f*0.5; player.head.position.z = 0.1 + f*0.25; player.head.rotation.x = f*0.4; }
            else { isAttacking = false; player.visual.position.z = 0; player.head.position.z = 0.1; player.head.rotation.x = 0; }
        }
        for (const id of Object.keys(otherPlayers)) {
            const op = otherPlayers[id];
            if (op.isAttacking) {
                op.attackAnimTime += 12*dt; const f = Math.sin(op.attackAnimTime*Math.PI);
                if (op.attackAnimTime <= 1) { op.visual.position.z = f*0.5; op.head.position.z = 0.1 + f*0.25; op.head.rotation.x = f*0.4; }
                else { op.isAttacking = false; op.visual.position.z = 0; op.head.position.z = 0.1; op.head.rotation.x = 0; }
            }
        }
        const fy = getFloorY(rabbit.position.x, rabbit.position.y, rabbit.position.z);
        velocityY -= gravity*60*dt; rabbit.position.y += velocityY*dt;
        if (rabbit.position.y <= fy) { rabbit.position.y = fy; velocityY = 0; jumpCount = 0; }
        if (isOnlineMode) for (const id of Object.keys(otherPlayers)) {
            const o = otherPlayers[id].mesh;
            if (rabbit.position.distanceTo(o.position) < 1.2 && rabbit.position.distanceTo(o.position) > 0.01) {
                const a = Math.atan2(rabbit.position.x - o.position.x, rabbit.position.z - o.position.z);
                rabbit.position.x += Math.sin(a)*0.05; rabbit.position.z += Math.cos(a)*0.05;
                o.position.x -= Math.sin(a)*0.05; o.position.z -= Math.cos(a)*0.05;
            }
        }
